...
#'
#' @param file file name or file path of the ".txt" file of rulesets generated by DeskstopGARP
...
#' @param pathB file path to the directory for ruleset grids produced by DesktopGARP
...
extractRules <- function(file, table, pathA, pathB, project){
  ...
  for (k in seq(1,length(modelRule$bestmodel))){
    for (i in seq(1,length(list),by=number)){ # loop for model
                    ...
                    if (grepl("\\[|\\]|,",ruleset[n])==TRUE ){
                      ...
                      if (grepl("\\=",ruleset[n])==TRUE){
                        ...
                        if (grepl(',',temp1[2])==TRUE){
                          ...
                          modelNumber <- append(modelNumber,modelNum)
                          RuleNumber <- append(RuleNumber,ruleset[a])
                          #print(temp2[1])
                          if (temp2[2] != ''){
                    ...
                    if (grepl("\\*",ruleset[n])==TRUE){
                      ...
                      if (as.numeric(temp1[2]) != 0){
                        ...
                        for (y in seq(1:length(files))){
                          ...
                          rfiles <- list.files(rdir2_1, pattern = 'rset_\\d+_0')
                          for (w in seq(1:length(rfiles))){
                            if (grepl(".aux",rfiles[w])==FALSE){
                              rdir2ID <- regmatches(rfiles[w], regexpr('\\d+_',rfiles[w]))
                              rdir2ID <- regmatches(rdir2ID, regexpr('\\d+',rdir2ID))
                              if (as.numeric(rdir2ID) == as.numeric(modelNum)){
                                rsetRa <- paste(rdir2_1, rfiles[w], sep = '')
                                rsetRa <- raster(rsetRa)
                                ZonalMin <- ZonalStat(PhyPa,rsetRa,'min')
                                ZonalMax <- ZonalStat(PhyPa,rsetRa,'max')
                                for(z in seq(1:length(ZonalMin$zones))){
                                  if (ZonalMin$zones[z]==ruleset[a]){
                                    RuleName <- append(RuleName,temp1[1])
                                    RuleType <- append(RuleType,"logit")
                                    modelNumber <- append(modelNumber,modelNum)
                                    RuleNumber <- append(RuleNumber,ruleset[a])
                                    Minimum <- append(Minimum,ZonalMin$min[z])
                                    Maximum <- append(Maximum,ZonalMax$max[z])
                                  }
                                }
  ...
  rangelogit <- data.frame(RuleName,Minimum,Maximum,RuleType,RuleNumber,modelNumber)
  ...
  for(t in 1:dim(rangelogit)[1]){
    ...
    if(grepl("\\[",rangelogit$Minimum[t])==TRUE){
      rangelogit$Minimum[t] <- as.numeric(gsub("\\[","",rangelogit$Minimum[t]))
    }else if(grepl("\\]",rangelogit$Maximum[t])==TRUE){
      rangelogit$Maximum[t] <- as.numeric(gsub("\\]","",rangelogit$Maximum[t]))
    }
  }
