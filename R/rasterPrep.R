#' Prepares input rasters for DesktopGARP
#'
#' \code{rasterPrep} Desktop GARP requires all input environmental layers to be of the same spatial resolution and extent and saved as an ".asc". This function resamples input rasters to the same cell size and extent of a sample raster.
...
rasterPrep <- function(file.path, res.raster = NULL, cells = NULL, mask = NULL, rescale = FALSE, output.path){
  ...
  for(i in 1:length(list.rasters)){
    ...
    r <- read.asc(paste(output.path, name, "_resample.asc", sep=""))
    write.asc(r, paste(output.path, name, "_resample.asc", sep=""))
  }
}
