...
#'
#' \code{centroid} DesktopGARP records the instance of a presence location in a grid cell, and therefore does not distinguish between multiple locations within a single cell. This function
#' returns the centroid location of all grid cells that contain a presence location, which circumvents multiple locations. Useful when input rasters cells are large or
#' when sampling was unevenly clustered. Uses the cell size and extent of user-defined raster to generate centroid coordinates of all cells that contain presence locations.
#'
...
#'
#' @details \code{x} should be a raster that represents the spatial resolution and extent of all environmental layers that will be inputted into DesktopGARP. Recommended to use a raster output by \code{\link{rasterPrep}}.
#'
#' @examples
#' set.seed(0)
