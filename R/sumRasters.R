#' Sums rasters of the best subset models output by DesktopGARP
#'
#' \code{sumRasters} DesktopGARP outputs binary presence/absence grids for the sample area and selects the best models based on user-defined qualifications. This function
...
#'
#' @param file.path path to the location of output rasters to sum (usually found in "GARPruns" folder output by DesktopGARP)
...
sumRasters <- function(file.path, output = FALSE, output.path = NULL, format = NULL, over.write = FALSE){
  ...
  list.files <- list.files(path = paste(file.path), full.names = TRUE)
